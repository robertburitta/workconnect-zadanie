...
import { XIcon, InfoIcon, TriangleAlertIcon, OctagonXIcon, Loader2Icon, CheckIcon } from "lucide-react";
...
function ToastContent({ className, ...props }: ToastPrimitive.Content.Props) {
  return (
    <ToastPrimitive.Content
      ...
      className={cn(
        "flex min-h-13 items-center gap-3 overflow-hidden px-4 py-3 transition-opacity duration-250 ease-[cubic-bezier(0.22,1,0.36,1)] data-behind:opacity-0 data-expanded:opacity-100",
        className,
      )}
      {...props}
    />
  ...
  return toasts.map((toastItem) => (
    <Toast key={toastItem.id} toast={toastItem}>
      <ToastContent>
        ...
        <ToastAction />
        {toastItem.type !== "success" && <ToastClose />}
      </ToastContent>
    </Toast>
  ));
}
